    ...
    <div className="min-h-screen w-full bg-neutral-950 flex flex-col items-center justify-center p-4">
      ...
      <footer className="w-full flex justify-around items-center py-3 bg-neutral-950 fixed bottom-0 left-0 z-20">
        {abas.map(({ value, icon: Icon }) => (
          <button key={value} onClick={() => setAbaAtiva(value)} className={`flex flex-col items-center justify-center px-2 py-1 rounded transition-colors ${abaAtiva === value ? "text-[#ca8a04]" : "text-neutral-500"} focus:outline-none focus:ring-0`} style={{ background: "none", border: "none" }} aria-label={value}>
